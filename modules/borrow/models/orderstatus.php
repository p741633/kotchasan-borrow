            ...
            if (Login::checkPermission($login, 'can_approve_borrow')) {
                try {
                    ...
                    if (!$index) {
                            ...
                            if ($amount > 0) {
                                if ($amount + $index->amount > $index->num_requests) {
                                    ...
                                    if ($amount > $index->stock) {
                                        // สต๊อคไม่เพียงพอ
                                        $ret['ret_amount'] = Language::replace('There is not enough :name (remaining :stock :unit)', [':name' => $index->topic, ':stock' => $index->stock, ':unit' => $index->unit]);
                                    } else {
                                        // ตัดสต๊อค
                                        $save = ['amount' => $index->amount + $amount, 'status' => $status];
                                        // อัปเดตรายการ
                        ...
                        if (empty($ret)) {
                            if (isset($stock)) {
                                $this->db()->update($this->getTableName('inventory_items'), ['product_no', $index->product_no], $stock);
                            }
                            if (isset($save)) {
                                // บันทึก borrow_items
                                $this->db()->update($this->getTableName('borrow_items'), [
